...
// E2E mocha reports are typically output as structured JSON/results depending on reporter output.
// We will read results from excelReporter output or the stored results array and export it to HTML.
function generateHtml(results, outputPath) {
    let total = results.length;
    let passed = results.filter(r => r.Status === 'PASSED').length;
    let failed = total - passed;
    let totalDuration = results.reduce((acc, r) => acc + parseInt(r.Duration || 0), 0);
    
    let testRowsHtml = '';
    results.forEach((result, index) => {
        let statusClass = result.Status === 'PASSED' ? 'status-passed' : 'status-failed';
        const errorHtml = result.Error ? `<div class="error-msg">${escapeHtml(result.Error)}</div>` : 'None';

        testRowsHtml += `
        <tr>
            <td>${index + 1}</td>
            <td><span class="badge badge-info">TC-WEB-${index + 1}</span></td>
            <td><strong>${escapeHtml(result.TestName)}</strong></td>
            <td><span class="category-tag">${escapeHtml(result.Suite || 'General')}</span></td>
            <td>${escapeHtml(result.TestingType || 'Web Testing')}</td>
...
<head>
    ...
    <title>BrainBattle Selenium Web Test Execution Report</title>
    <style>
        :root {
            --bg-color: #0f0f1a;
...
<body>
    <div class="container">
        <header>
            <h1>🌐 BrainBattle Selenium Web E2E Report</h1>
            <p style="margin: 0; color: var(--text-muted);">Live Website E2E & Browser Automation Suite</p>
            <div class="meta-grid">
                <div class="meta-item">
                ...
                <div class="meta-item">
                    ...
                    <div class="meta-value">Web (Selenium + Mocha)</div>
                </div>
                <div class="meta-item">
        ...
        <h2 style="color: var(--primary); margin-bottom: 15px;">🧪 Web Test Details</h2>
        <table>
            <thead>
                <tr>
    ...
    console.log(`✅ HTML execution report saved → ${outputPath}`);
}
